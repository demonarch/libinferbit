...
extern "C" int ib_metal_rec_matmul_w4a8_blk32_dr_a32_silu_fp32_in(
    ib_metal_recorder *rec,
    const void *gate_fp32,
    const void *up_fp32,
    const void *weights,
    ...
    int M, int N)
{
    if (!rec || !gate_fp32 || !up_fp32 || !weights || !w_scales || !out
        || M <= 0 || N <= 0) return -1;
    if ((N % 32) != 0) return -1;
    id<MTLComputePipelineState> ps = get_pipeline(rec->ctx, "matmul_w4a8_blk32_dr_a32_silu");
    if (!ps) return -1;
    ...
    id<MTLBuffer> b_ws  = rec_pick(rec->ctx, w_scales);
    id<MTLBuffer> b_out = rec_pick(rec->ctx, out);
    if (!b_g || !b_u || !b_w || !b_ws || !b_out) return -1;
    uint M_u = (uint)M, N_u = (uint)N;
    id<MTLComputeCommandEncoder> enc = [rec->cb computeCommandEncoder];
    [enc setComputePipelineState:ps];
    [enc setBuffer:b_w   offset:0 atIndex:0];
    [enc setBuffer:b_ws  offset:0 atIndex:1];
    [enc setBuffer:b_g   offset:0 atIndex:2];
    [enc setBuffer:b_u   offset:0 atIndex:3];
    [enc setBuffer:b_out offset:0 atIndex:4];
    [enc setBytes:&M_u length:sizeof(M_u) atIndex:5];
    [enc setBytes:&N_u length:sizeof(N_u) atIndex:6];
    const NSUInteger SIMDS_PER_TG = 4;
...
/* PQv2 batched simdmat tg16 — small-TG / high-occupancy variant.
 * 16×32 output tile, 8 SIMDgroups per TG, 6 KB TG memory. Targets
 * gather-bound throughput by keeping per-TG resource use low so more
 * TGs are co-resident per shader core. M%16 + B%32 + N%64. */
extern "C" int ib_metal_rec_matmul_pqv2_batched_simdmat_tg16(ib_metal_recorder *rec,
                                                                const void *row_scale_fp16,
                                                                const void *cb_fp16,
                                                                const void *indices_u8,
                                                                const void *x_fp32,
                                                                ...
                                                                int G, int n_subchunks)
{
    if (!rec || !row_scale_fp16 || !cb_fp16 || !indices_u8 || !x_fp32 || !out_fp32
        || B <= 0 || M <= 0 || N <= 0 || G <= 0 || n_subchunks <= 0) return -1;
    if ((N % G) != 0) return -1;
    if ((M % 16) != 0 || (B % 32) != 0 || (N % 64) != 0) return -2;
    id<MTLComputePipelineState> ps = get_pipeline(rec->ctx, "matmul_pqv2_batched_simdmat_tg16");
    if (!ps) return -1;
    ...
    uint M_u = (uint)M, N_u = (uint)N, B_u = (uint)B, G_u = (uint)G, ns_u = (uint)n_subchunks;
    id<MTLComputeCommandEncoder> enc = [rec->cb computeCommandEncoder];
    [enc setComputePipelineState:ps];
    [enc setBuffer:b_rs offset:ors atIndex:0];
    [enc setBuffer:b_cb offset:ocb atIndex:1];
    ...
    [enc setBytes:&N_u  length:sizeof(N_u)  atIndex:6];
    [enc setBytes:&B_u  length:sizeof(B_u)  atIndex:7];
    [enc setBytes:&G_u  length:sizeof(G_u)  atIndex:8];
    [enc setBytes:&ns_u length:sizeof(ns_u) atIndex:9];

    NSUInteger tg_W_bytes = 16 * 64 * sizeof(uint16_t);
    NSUInteger tg_A_bytes = 32 * 64 * sizeof(uint16_t);
    [enc setThreadgroupMemoryLength:tg_W_bytes atIndex:0];
    [enc setThreadgroupMemoryLength:tg_A_bytes atIndex:1];

    NSUInteger n_tg_x = ((NSUInteger)M + 15) / 16;
    NSUInteger n_tg_y = ((NSUInteger)B + 31) / 32;
...
/* PQv2 batched simdmat tg64 — 64×32 output tile, 32 SIMDgroups per
 * TG. M%64 + B%32 + N%64. Returns -2 if shape doesn't fit. */
extern "C" int ib_metal_rec_matmul_pqv2_batched_simdmat_tg64(ib_metal_recorder *rec,
                                                                const void *row_scale_fp16,
                                                                const void *cb_fp16,
                                                                const void *indices_u8,
                                                                const void *x_fp32,
                                                                ...
                                                                int G, int n_subchunks)
{
    if (!rec || !row_scale_fp16 || !cb_fp16 || !indices_u8 || !x_fp32 || !out_fp32
        || B <= 0 || M <= 0 || N <= 0 || G <= 0 || n_subchunks <= 0) return -1;
    if ((N % G) != 0) return -1;
    if ((M % 64) != 0 || (B % 32) != 0 || (N % 64) != 0) return -2;
    id<MTLComputePipelineState> ps = get_pipeline(rec->ctx, "matmul_pqv2_batched_simdmat_tg64");
    if (!ps) return -1;
    ...
    NSUInteger tg_W_bytes = 64 * 64 * sizeof(uint16_t);
    NSUInteger tg_A_bytes = 32 * 64 * sizeof(uint16_t);
    [enc setThreadgroupMemoryLength:tg_W_bytes atIndex:0];
    [enc setThreadgroupMemoryLength:tg_A_bytes atIndex:1];

    NSUInteger n_tg_x = ((NSUInteger)M + 63) / 64;
    NSUInteger n_tg_y = ((NSUInteger)B + 31) / 32;
    [enc dispatchThreadgroups:MTLSizeMake(n_tg_x, n_tg_y, 1)
...
/* PQv2 batched simdmat with B_BLOCK=16. Halves the W loads vs b8 by
 * covering 16 batch tokens per TG. Requires M%32, B%16, N%64. */
extern "C" int ib_metal_rec_matmul_pqv2_batched_simdmat_b16(ib_metal_recorder *rec,
                                                              const void *row_scale_fp16,
                                                              const void *cb_fp16,
                                                              const void *indices_u8,
                                                              const void *x_fp32,
                                                              ...
                                                              int G, int n_subchunks)
{
    if (!rec || !row_scale_fp16 || !cb_fp16 || !indices_u8 || !x_fp32 || !out_fp32
        || B <= 0 || M <= 0 || N <= 0 || G <= 0 || n_subchunks <= 0) return -1;
    if ((N % G) != 0) return -1;
    if ((M % 32) != 0 || (B % 16) != 0 || (N % 64) != 0) return -2;
    id<MTLComputePipelineState> ps = get_pipeline(rec->ctx, "matmul_pqv2_batched_simdmat_b16");
    if (!ps) return -1;
    ...
    [enc setThreadgroupMemoryLength:(32 * 64 * sizeof(uint16_t)) atIndex:0];
    [enc setThreadgroupMemoryLength:(16 * 64 * sizeof(uint16_t)) atIndex:1];

    NSUInteger n_tg_x = ((NSUInteger)M + 31) / 32;
    NSUInteger n_tg_y = ((NSUInteger)B + 15) / 16;
    [enc dispatchThreadgroups:MTLSizeMake(n_tg_x, n_tg_y, 1)
          threadsPerThreadgroup:MTLSizeMake(8 * 32, 1, 1)];
    [enc endEncoding];
    return 0;
...
/* PQv2 batched simdmat with B_BLOCK=8 — works at any B%8==0. Closes
 * the prefill speed cliff for prompts not sized to 32. Returns -2 if
 * shape doesn't satisfy M%32, B%8, N%64. */
extern "C" int ib_metal_rec_matmul_pqv2_batched_simdmat_b8(ib_metal_recorder *rec,
                                                              const void *row_scale_fp16,
                                                              const void *cb_fp16,
                                                              const void *indices_u8,
                                                              const void *x_fp32,
                                                              ...
                                                              int G, int n_subchunks)
{
    if (!rec || !row_scale_fp16 || !cb_fp16 || !indices_u8 || !x_fp32 || !out_fp32
        || B <= 0 || M <= 0 || N <= 0 || G <= 0 || n_subchunks <= 0) return -1;
    if ((N % G) != 0) return -1;
    if ((M % 32) != 0 || (B % 8) != 0 || (N % 64) != 0) return -2;
    id<MTLComputePipelineState> ps = get_pipeline(rec->ctx, "matmul_pqv2_batched_simdmat_b8");
    if (!ps) return -1;
    ...
    NSUInteger tg_W_bytes = 32 * 64 * sizeof(uint16_t);
    NSUInteger tg_A_bytes = 8  * 64 * sizeof(uint16_t);
    [enc setThreadgroupMemoryLength:tg_W_bytes atIndex:0];
    [enc setThreadgroupMemoryLength:tg_A_bytes atIndex:1];

    NSUInteger n_tg_x = ((NSUInteger)M + 31) / 32;
    NSUInteger n_tg_y = ((NSUInteger)B + 7) / 8;
    [enc dispatchThreadgroups:MTLSizeMake(n_tg_x, n_tg_y, 1)
          threadsPerThreadgroup:MTLSizeMake(4 * 32, 1, 1)];
    [enc endEncoding];
    return 0;
}

/* PQv2 batched simdmat (prefill): uses Apple's simdgroup_matrix HW.
 * Requires M % 32 == 0, B % 32 == 0, N % 64 == 0. Returns -2 (not
 * applicable) when the shape doesn't meet the constraints — caller
 * should fall back to the SIMD-coop batched kernel. */
extern "C" int ib_metal_rec_matmul_pqv2_batched_simdmat(ib_metal_recorder *rec,
                                                          const void *row_scale_fp16,
                                                          const void *cb_fp16,
                                                          const void *indices_u8,
                                                          const void *x_fp32,
                                                          ...
                                                          int G, int n_subchunks)
{
    if (!rec || !row_scale_fp16 || !cb_fp16 || !indices_u8 || !x_fp32 || !out_fp32
        || B <= 0 || M <= 0 || N <= 0 || G <= 0 || n_subchunks <= 0) return -1;
    if ((N % G) != 0) return -1;
    if ((M % 32) != 0 || (B % 32) != 0 || (N % 64) != 0) return -2;
    id<MTLComputePipelineState> ps = get_pipeline(rec->ctx, "matmul_pqv2_batched_simdmat");
    if (!ps) return -1;
    ...
    /* 32×64 W tile fp16 = 4 KB; same for A. */
    NSUInteger tg_W_bytes = 32 * 64 * sizeof(uint16_t);
    NSUInteger tg_A_bytes = 32 * 64 * sizeof(uint16_t);
    [enc setThreadgroupMemoryLength:tg_W_bytes atIndex:0];
    [enc setThreadgroupMemoryLength:tg_A_bytes atIndex:1];

    /* Grid: (M/32, B/32, 1). TG: 16 SIMDgroups × 32 lanes = 512 threads. */
    NSUInteger n_tg_x = ((NSUInteger)M + 31) / 32;
    NSUInteger n_tg_y = ((NSUInteger)B + 31) / 32;
    [enc dispatchThreadgroups:MTLSizeMake(n_tg_x, n_tg_y, 1)
          threadsPerThreadgroup:MTLSizeMake(16 * 32, 1, 1)];
    [enc endEncoding];
    return 0;
}
